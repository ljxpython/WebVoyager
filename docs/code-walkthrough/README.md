...
这个目录记录老王对当前 WebVoyager 项目的代码上手笔记。目标不是复述论文，而是把这个仓库怎么跑、怎么观测网页、怎么让模型决策、怎么保存轨迹、怎么做评测讲清楚，方便后续拿它评测你的 Agent 项目。
...
4. [评测接入指南](04-evaluation-adaptation.md)：把它改造成评测你自己的 Web Agent 的工具链。
